# -*- coding: utf-8 -*-
# @Time    : 2022/7/11 14:21
# @Author  : Kenny Zhou
# @FileName: kmeans_hue.py
# @Software: PyCharm
...
def color_cluster(img_file, k=3):
    ...
    return np.array(list(label_value)), np.array(label_count), rgb_array, ch_score, pixel_label,hsv_array
...
def opencv2hsv(input):
    input = np.array(input.astype(np.float))
    input[:,:,0] = input[:,:,0] / 180 * 360.
    input[:,:,1] = input[:,:,1] / 255
    input[:,:,2] = input[:,:,2] / 255
    return input
...
if __name__ == '__main__':
    img_file = r'/Volumes/data/inbox/朱锦豫（不用归档）/水体颜色/水体颜色/黑色/u=1723590134,963334344&fm=253&fmt=auto&app=138&f=JPEG.jpg'
    max_score = -1000
    renders = []
    hsv_labels = []
    best_k = -1
